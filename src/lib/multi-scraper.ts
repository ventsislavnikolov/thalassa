...
export class MultiHotelScraper {
  private lastRawHtml: Map<string, string> = new Map();
  ...
  async fetchAllHotels(
    ...
    for (const hotel of hotels) {
      try {
        const response = await this.fetchCalendarData(params, hotel.id);
  ...
  async findLowestPricesAllHotels(
    params: SearchParams,
    ...
    for (const hotel of hotels) {
      ...
      for (let i = 1; i < monthsToCheck; i++) {
        ...
        try {
          const response = await this.fetchCalendarData(searchParams, hotel.id);
  ...
  private buildFormData(params: SearchParams): URLSearchParams {
  ...
  getLastRawHtml(hotelId: string): string {
    return this.lastRawHtml.get(hotelId) || "";
  }
}
